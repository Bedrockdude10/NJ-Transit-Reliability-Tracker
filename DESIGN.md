...
- **Measured** — the independent per-train metrics (OTP at strict thresholds,
  delay distribution, time-of-day heatmaps, worst trips, station delays,
  connection reliability), derived from the live GTFS-Realtime feed. There is
  **no synthetic data** — before the API was connected these were seeded to
  demonstrate the methodology, but the seed has been removed. The metrics are
  now real, and honestly empty/sparse until enough live data accrues.

Rules enforced in the UI:

1. **Never show modeled where real exists.** Trips operated / cancellations /
   cancellation rate come from `NjtOfficialComparison` (summed from official
   metrics), not the per-train event counts.
2. **Don't present an empty measurement as a number.** With the seed gone, the
   independent metrics start empty. The UI must show "measuring since \<date\> —
   accruing" (not `0%`) until there's a meaningful sample.

**Follow-up (in progress):** the `<ModeledBadge>`/`<ModeledBanner>` labels and
"until live data is connected" copy predate the live feed and are now stale —
they should flip to a "live · measuring since \<date\>" treatment. This
real/measured seam is the piece to finish.

...
- **New field on a shared dataset:** add it to the interface in `shared/src/domain.ts` or `predictions.ts` → `npm run generate:contract` → `npm run emit:data-contract` → in `njt-delay-modeling`, `uv run python scripts/sync_contract.py` and commit both. Both CIs fail until you do. A *breaking* change is a new `contract/v2/`, never an edit.

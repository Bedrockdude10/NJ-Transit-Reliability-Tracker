...
- **New screen:** a panel in `app/screens/<Name>Panel.tsx`, added as a tab to one of the five pages (`app/app/{index,network,trips,analysis,about}`). The nav is five destinations, not one per panel; `app/app/<route>/index.tsx` is only for a genuinely new destination, and then it needs a link in `app/components/NavBar.tsx`. A panel renders a fragment, not its own `<Screen>` — `TabbedPage` provides that, and holds the selected tab in the URL (`?tab=`) via `useTab`. Old paths stay as `RedirectTo` stubs onto their tab. Fetch via `app/lib/api.ts` (`useApi` / `useApis` / `useLiveApi`, thin wrappers over TanStack Query). Client methods return an `ApiQuery` — a cache key plus how to run it — not a promise, so the key is always the URL and can never disagree with the request.
...
- **Predictions round-trip: model → R2 → SQLite → API → app.** `njt-delay-modeling` writes `predictions/service_date=…/predictions.jsonl.gz`; `npm run import:predictions` validates every row against the generated `delayPredictionSchema` and lands it in the `predictions` table (all-or-nothing per day, so a half-written upload cannot appear as a complete one); `GET /predictions?date=` serves it; `/predictions` renders it. The supervisor imports hourly once `NJT_PREDICTION_IMPORT_ENABLED=true`. **Predictions are the only unobserved numbers on the site**, so the screen names the model and run, keeps the model's average miss on the page, and says "no predictions yet" plainly rather than rendering an empty table. It does **not** lead with the error: a rider opens the screen to find one train, so the legs come first and the track record sits under "How far to trust this". Per-model accuracy over time is `GET /models`, which is deliberately unrendered — an operator's question, not a rider's.
...
- **Comments carry what the code can't.** A comment earns its place by recording a constraint a reader would otherwise undo: why the obvious implementation is wrong here, where a magic number came from, an upstream bug, a spec citation. Not history ("used to be…"), not what the code plainly says, not architecture — that lives in this file. Three lines is a lot; fifteen is a bug. Rationale belongs in the commit message and the PR, which is where someone asking "why" will actually look.
- **`@unit` and `@format int` are load-bearing** — `emit-data-contract` reads them out of `domain.ts`/`predictions.ts`. Never strip them. The `description:` strings in `datasets.ts` and `emit-data-contract.ts` are contract *payload*, not commentary.
- **ts-to-zod copies JSDoc** into `*.zod.ts`. Trim the source, then `npm run generate:contract`.
- **`npx tsx scripts/check-comments-only.ts`** proves a diff touched only comments: it strips comments from both sides and compares the emitted code. Note SQL `--` comments inside migration template literals are *code* to it, and an applied migration is never edited anyway.

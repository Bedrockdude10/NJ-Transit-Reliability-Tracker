...
- **`npm run sweep:archive`** drains `raw_snapshots` to object storage (hourly Parquet objects, digest-verified before deletion) and is what keeps the volume from filling — it deliberately does not `VACUUM`, so the file stops growing rather than shrinking. Sweep before enabling Litestream: replicating a 3.7 GB database starved the API and caused an outage.
- **`npm run export:events`** publishes derived events to object storage as Parquet (DuckDB, one hive-partitioned object per service date) for the Python modelling repo. The column list in `pipeline/src/archive/export-events.ts` is checked against the contract by a test — add a contract field and the export fails until it's included.
- **The object-storage contract lives in `contract/v1/*.json`** — generated by `npm run emit:data-contract` from `domain.ts` / `predictions.ts` via Zod. It is the seam with the Python modelling repo (`njt-delay-modeling`), which generates pydantic models from it. TypeScript is the schema authority for every data contract; a breaking change means a new `contract/v2/`, never an edit. Mark genuinely-integer fields `@format int` — TS has no integer type, and without it epoch fields become floats downstream.
